...
import { ArrowLeft, Clock, Award, CheckCircle2, Star, Play, Heart, ArrowRight, Zap } from 'lucide-react';
import Link from 'next/link';
import { type Lang } from '../../../lib/i18n';

export default function AtencionCliente5StarPage() {
  const [lang, setLang] = React.useState<Lang>('es');

  const courseData = {
    es: {
      title: 'Atención al Cliente 5 Estrellas',
      category: 'Ventas y Customer Experience',
      categoryIcon: '🤝',
      price: 26,
      description: 'Ofrecé experiencias memorables que fidelizan. Aprendé las mejores prácticas de atención al cliente para convertir clientes en fans de tu marca.',
      highlights: [
      ...
      testimonials: [
        {
          name: 'María José L.',
          quote: 'Transformé la forma en que atiendo a mis clientes. Ahora recibo muchas más recomendaciones.',
          role: 'Dueña de Negocio'
        },
        {
          name: 'Tomás R.',
          quote: 'El módulo de manejo de quejas es oro puro. Muy aplicable.',
          role: 'Supervisor de Call Center'
        },
        {
          name: 'Luciana F.',
          quote: 'Aprendí a convertir clientes enojados en fans. Increíble.',
          role: 'Customer Success Manager'
        }
      ],
      relatedCourses: [
        { id: 'ventas-101', title: 'Ventas 101', description: 'Complementá con técnicas de ventas' },
        { id: 'objeciones-bajo-control', title: 'Objeciones bajo Control', description: 'Manejá objeciones con confianza' },
        { id: 'comunicacion-efectiva-pro', title: 'Comunicación Efectiva Pro', description: 'Mejorá tu comunicación' }
      ]
    },
    en: {
      ...
      categoryIcon: '🤝',
      price: 26,
      description: 'Offer memorable experiences that build loyalty. Learn the best customer service practices to turn customers into fans of your brand.',
      highlights: [
      ...
      testimonials: [
        {
          name: 'María José L.',
          quote: 'I transformed the way I serve my customers. Now I receive many more referrals.',
          role: 'Business Owner'
        },
        {
          name: 'Tomás R.',
          quote: 'The complaint handling module is pure gold. Very applicable.',
          role: 'Call Center Supervisor'
        },
        {
          name: 'Luciana F.',
          quote: 'I learned to turn angry customers into fans. Incredible.',
          role: 'Customer Success Manager'
        }
      ],
      relatedCourses: [
        { id: 'ventas-101', title: 'Sales 101', description: 'Complement with sales techniques' },
        { id: 'objeciones-bajo-control', title: 'Objections Under Control', description: 'Handle objections with confidence' },
        { id: 'comunicacion-efectiva-pro', title: 'Effective Communication Pro', description: 'Improve your communication' }
      ]
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      <section style={{ padding: '0', background: '#F8FAFC' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1rem' }}>
          <div style={{ background: 'linear-gradient(135deg, #E2E8F0 0%, #CBD5E1 100%)', borderRadius: '0 0 1rem 1rem', height: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '-2rem', position: 'relative', zIndex: 10 }}>
            <div style={{ textAlign: 'center', color: '#64748B' }}><Heart size={64} style={{ marginBottom: '1rem', opacity: 0.5 }} /><p style={{ fontSize: '1rem' }}>{lang === 'es' ? 'Vista previa del curso' : 'Course preview'}</p></div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '4rem 1rem', background: 'linear-gradient(135deg, #3B82F6 0%, #8B5CF6 100%)', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: 700, color: 'white', marginBottom: '1rem' }}>{lang === 'es' ? '¿Listo para crear experiencias memorables?' : 'Ready to create memorable experiences?'}</h2>
          <p style={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.9)', marginBottom: '2rem' }}>{lang === 'es' ? 'Empezá hoy y transformá la forma en que atendés a tus clientes.' : 'Start today and transform the way you serve your customers.'}</p>
          <button style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 2.5rem', fontSize: '1.125rem', fontWeight: 700, color: '#3B82F6', background: 'white', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', boxShadow: '0 10px 25px rgba(0, 0, 0, 0.2)' }}>{lang === 'es' ? 'Comprar por USD $26' : 'Buy for USD $26'}<ArrowRight size={20} /></button>
        </div>
      </section>
    </div>
  );
