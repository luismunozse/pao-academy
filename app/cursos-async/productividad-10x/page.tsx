...
import { ArrowLeft, Clock, Award, CheckCircle2, Star, Play, Target, ArrowRight, Zap } from 'lucide-react';
import Link from 'next/link';
import { type Lang } from '../../../lib/i18n';

export default function Productividad10XPage() {
  const [lang, setLang] = React.useState<Lang>('es');

  const courseData = {
    es: {
      title: 'Productividad 10X',
      category: 'Mindset y Desarrollo Personal',
      categoryIcon: '🧠',
      price: 34,
      description: 'Diseñá rutinas que multipliquen tus resultados diarios. Sistemas probados para maximizar tu rendimiento personal y profesional.',
      highlights: ['Sistemas de productividad', 'Gestión del tiempo efectiva', 'Hábitos de alto rendimiento', 'Eliminación de distracciones'],
      modules: ['Diagnóstico de productividad actual', 'Sistemas: GTD, Pomodoro, Time Blocking', 'Rutinas matutinas y nocturnas', 'Deep Work y estado de flow', 'Herramientas digitales productivas', 'Diseño de tu sistema personal'],
      duration: '6 horas de contenido', format: '100% on-demand', access: 'Acceso de por vida', certificate: 'Certificado digital incluido',
      testimonials: [
        { name: 'Ignacio M.', quote: 'Pasé de trabajar 10 horas a 6 haciendo más cosas importantes.', role: 'Director de Proyectos' },
        { name: 'Sofía L.', quote: 'El time blocking cambió mi vida. Ahora tengo tiempo para todo.', role: 'Emprendedora' },
        { name: 'Federico R.', quote: 'Sistemas simples que realmente funcionan. Muy recomendado.', role: 'Desarrollador' }
      ],
      relatedCourses: [
        { id: 'notion-productividad', title: 'Notion para la Productividad', description: 'Organiza con Notion' },
        { id: 'comunicacion-asertiva', title: 'Comunicación Asertiva', description: 'Comunica mejor' },
        { id: 'trabajo-equipo-remoto', title: 'Trabajo en Equipo Remoto', description: 'Colabora efectivamente' }
      ]
    },
    en: {
      ...
      categoryIcon: '🧠',
      price: 34,
      description: 'Design routines that multiply your daily results. Proven systems to maximize your personal and professional performance.',
      highlights: ['Productivity systems', 'Effective time management', 'High-performance habits', 'Distraction elimination'],
      modules: ['Current productivity diagnosis', 'Systems: GTD, Pomodoro, Time Blocking', 'Morning and evening routines', 'Deep Work and flow state', 'Productive digital tools', 'Design your personal system'],
      duration: '6 hours of content', format: '100% on-demand', access: 'Lifetime access', certificate: 'Digital certificate included',
      testimonials: [
        { name: 'Ignacio M.', quote: 'I went from working 10 hours to 6 doing more important things.', role: 'Project Director' },
        { name: 'Sofía L.', quote: 'Time blocking changed my life. Now I have time for everything.', role: 'Entrepreneur' },
        { name: 'Federico R.', quote: 'Simple systems that really work. Highly recommended.', role: 'Developer' }
      ],
      relatedCourses: [
        { id: 'notion-productividad', title: 'Notion for Productivity', description: 'Organize with Notion' },
        { id: 'comunicacion-asertiva', title: 'Assertive Communication', description: 'Communicate better' },
        { id: 'trabajo-equipo-remoto', title: 'Remote Teamwork', description: 'Collaborate effectively' }
      ]
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      <section style={{ padding: '0', background: '#F8FAFC' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1rem' }}>
          <div style={{ background: 'linear-gradient(135deg, #E2E8F0 0%, #CBD5E1 100%)', borderRadius: '0 0 1rem 1rem', height: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '-2rem', position: 'relative', zIndex: 10 }}>
            <div style={{ textAlign: 'center', color: '#64748B' }}><Target size={64} style={{ marginBottom: '1rem', opacity: 0.5 }} /><p style={{ fontSize: '1rem' }}>{lang === 'es' ? 'Vista previa del curso' : 'Course preview'}</p></div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '4rem 1rem', background: 'linear-gradient(135deg, #3B82F6 0%, #8B5CF6 100%)', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: 700, color: 'white', marginBottom: '1rem' }}>{lang === 'es' ? '¿Listo para multiplicar tu productividad?' : 'Ready to multiply your productivity?'}</h2>
          <p style={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.9)', marginBottom: '2rem' }}>{lang === 'es' ? 'Empezá hoy y transformá tu forma de trabajar.' : 'Start today and transform the way you work.'}</p>
          <button style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 2.5rem', fontSize: '1.125rem', fontWeight: 700, color: '#3B82F6', background: 'white', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', boxShadow: '0 10px 25px rgba(0, 0, 0, 0.2)' }}>{lang === 'es' ? 'Comprar por USD $34' : 'Buy for USD $34'}<ArrowRight size={20} /></button>
        </div>
      </section>
    </div>
  );
